...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2019/10/08 06:28:26 by wzei              #+#    #+#             */
/*   Updated: 2019/10/12 05:47:50 by wzei             ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "ft_printf.h"

static void	iter(t_ldbl_trv *trv, long double *num)
{
	if (trv->div > 1.0)
	{
...
static void	traverse_ld(t_ldbl_trv *trv, long double *num, t_pf *data)
{
	while (trv->prec >= 0)
	{
		if (trv->div > 1.0)
...
static char	sgn(long double *num, t_pf *data)
{
	if (*num < 0.0)
	{
		*num *= -1;
...
static void	rewind_digit(long double *num, t_ldbl_trv *trv)
{
	if (*num > 1.0)
	{
